...
export const sendFriendRequest = async (fromUserId, toUserId) => {
    ...
    const userCollection = await users();
    const toUser = await userCollection.findOne({ _id: new ObjectId(toUserId) });
    const fromUser = await userCollection.findOne({ _id: new ObjectId(fromUserId) });
...
export const acceptFriendRequest = async (currentUserId, fromUserId) => {
    ...
    const sender = await usersCollection.findOne({ _id: new ObjectId(fromUserId) });
